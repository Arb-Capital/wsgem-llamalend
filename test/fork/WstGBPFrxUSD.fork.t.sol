...
contract FrxUSDForkHarness is WstGBPFrxUSDMarketScript {
...
contract FrxUSDTGBPForkHarness is WstGBPMarketScript {
    function assertOracle(address oracle_) external {
        _assertOracle(IWsgemShimOracle(oracle_));
    }
}

/// @notice The wstGBP/frxUSD instance against live mainnet state: the real wsgem, both real
///         Chainlink feeds, and the real `LendFactory`.
/// @dev What the unit suite cannot show. Mocks prove the composition is the composition it says it
///      is; only this proves the addresses in `script/WstGBPFrxUSD.s.sol` are the contracts they
...
contract WstGBPFrxUSDForkTest is Test {
    WstGBPFrxUSDMarketScript internal cfg;
    ...
    function setUp() public {
        ...
        cfg          = new WstGBPFrxUSDMarketScript();
        WSGEM        = cfg.WSGEM();
        GEM          = cfg.GEM();
        FACTORY      = cfg.FACTORY();
    ...
    function test_theBorrowedTokenIsWhatTheConfigSays() public view {
        assertEq(IDecimals(cfg.BORROWED()).decimals(), 18, "frxUSD is 18 decimals");
    ...
    function test_theFrxUsdFeedIsFrxUsdAndNotFrax() public view {
        IChainlinkAggregator f_ = IChainlinkAggregator(cfg.BORROWED_QUOTE());
        assertEq(f_.decimals(), 8);

        (, int256 answer_,, uint256 updatedAt_,) = f_.latestRoundData();
        assertGt(answer_, 0.9e8);
        assertLt(answer_, 1.1e8);
        assertLt(block.timestamp - updatedAt_, cfg.MAX_FX_AGE(), "fresh at the pin");

        // The legacy FRAX feed answers materially differently. If that ever stops being true the
        // substitution stops being dangerous -- but so does the reason to care.
        (, int256 fraxAnswer_,,,) =
            IChainlinkAggregator(0xB9E1E3A9feFf48998E45Fa90847ed4D467E8BcfD).latestRoundData();
        assertTrue(fraxAnswer_ != answer_, "FRAX/USD is a different feed answering a different number");
    }
    ...
    function test_theOracleReportsTheComposedPrice() public {
        ...
        (, int256 usd_,,,) = IChainlinkAggregator(cfg.BORROWED_QUOTE()).latestRoundData();
        assertGt(gbp_, 0);
        assertGt(usd_, 0);

        // casting to 'uint256' is safe because both feeds were asserted positive above
        // forge-lint: disable-next-line(unsafe-typecast)
        uint256 gbpWad_ = uint256(gbp_) * 1e10;
        // forge-lint: disable-next-line(unsafe-typecast)
        uint256 usdWad_ = uint256(usd_) * 1e10;
        uint256 expected_ = (IWsgem(WSGEM).burncost() * gbpWad_) / usdWad_;

        assertEq(o_.price(), expected_, "burncost x GBP/USD / frxUSD/USD");
    ...
    ///      more than a dollar, so wstGBP is worth more frxUSD than tGBP.
    ...
    function test_aHaltedSterlingFeedFreezesRatherThanReverting() public {
        ...
        // No new round for longer than the bound allows. Note BOTH legs go stale together here:
        // they are both Chainlink push feeds on the same heartbeat, so a Chainlink-wide outage
        // freezes this market where it would only half-freeze the crvUSD one.
    ...
    function test_theMarketIsCreatedAndRegistered() public {
        ...
        assertEq(IVault(vault_).asset(), cfg.BORROWED(), "the vault takes deposits in frxUSD");
    ...
    function test_thisInstanceAcceptsItsOwnLiveOracle() public {
        FrxUSDForkHarness h_ = new FrxUSDForkHarness();
    ...
    function test_theFrxUsdInstancesLiveOracleIsRejected() public {
        WstGBPCrvUSDMarketScript crv_ = new WstGBPCrvUSDMarketScript();
        WsgemFxLlamalendOracle wrong_ = new WsgemFxLlamalendOracle(
            IWsgem(WSGEM),
            crv_.BORROWED(),
            IChainlinkAggregator(crv_.FX_FEED()),
            crv_.BORROWED_QUOTE(),
            crv_.BORROWED_QUOTE_KIND(),
            crv_.MAX_UPSIDE_SPEED(),
            crv_.MAX_FX_AGE()
        );
    ...
    function test_theFirstInstanceRejectsThisOracle() public {
        ...
        FrxUSDTGBPForkHarness h_ = new FrxUSDTGBPForkHarness();
